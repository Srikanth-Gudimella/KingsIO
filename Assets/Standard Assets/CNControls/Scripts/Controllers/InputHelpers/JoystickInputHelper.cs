...
//		public GameObject joyStickParent;//,joyStickBaseObj,stickObj;
//		public Vector3 joyStickDefaultPos;//,joyStickBaseDefaultPos,stickDefaultPos;
...
//			Debug.Log ("touchPosition="+(PointerEventDataCache.position));

			//this is used for floating type joystick
...
//					joyStickParent.transform.position = PointerEventDataCache.position;
//				}
				if (!RectTransformUtility.RectangleContainsScreenPoint (_linkedJoystick.BoostTouchZone,
					PointerEventDataCache.position, UiRootCamera) && !RectTransformUtility.RectangleContainsScreenPoint (_linkedJoystick.PauseTouchZone,
						PointerEventDataCache.position, UiRootCamera)) 
				{
					LastFingerId = 255;
					_linkedJoystick.OnPointerDown (PointerEventDataCache);
			...
			//this is used for LeftHanded or RightHanded type joystick
...
//            }
            ...
            {
                _linkedJoystick.OnPointerUp(PointerEventDataCache);
//				joyStickParent.transform.position = joyStickDefaultPos;
//				joyStickBaseObj.transform.position = joyStickBaseDefaultPos;
//				stickObj.transform.position = stickDefaultPos;

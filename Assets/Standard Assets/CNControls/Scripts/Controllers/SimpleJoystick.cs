...
namespace CnControls
{
...
#endif
    {
		...
		public RectTransform BoostTouchZone;
		public RectTransform PauseTouchZone;
		...
		private GameObject joyStickParent,stickObj;//joyStickBaseObj;
		private Vector3 joyStickDefaultPos,joyStickBaseDefaultPos,stickDefaultPos;
...
//			gameObject.GetComponent<JoystickInputHelper>().joyStickParent=this.gameObject;
//			gameObject.GetComponent<JoystickInputHelper>().joyStickBaseObj=JoystickBase.gameObject;
//			gameObject.GetComponent<JoystickInputHelper>().stickObj=Stick.gameObject;
//
//			gameObject.GetComponent<JoystickInputHelper>().joyStickDefaultPos=this.gameObject.transform.position;
//			gameObject.GetComponent<JoystickInputHelper>().joyStickBaseDefaultPos=JoystickBase.gameObject.transform.position;
//			gameObject.GetComponent<JoystickInputHelper>().stickDefaultPos=Stick.gameObject.transform.position;

#endif

			joyStickParent=this.gameObject;
			stickObj=Stick.gameObject;

			joyStickDefaultPos=this.gameObject.transform.localPosition;
			joyStickBaseDefaultPos=JoystickBase.gameObject.transform.localPosition;
			stickDefaultPos=Stick.gameObject.transform.localPosition;
        }
...
//            _stickTransform.anchoredPosition = stickAnchoredPosition;
        ...
        public void OnPointerUp(PointerEventData eventData)
        {
			...
			joyStickParent.transform.localPosition = joyStickDefaultPos;
			JoystickBase.transform.localPosition = joyStickBaseDefaultPos;
			stickObj.transform.localPosition = stickDefaultPos;

        }

        public void OnPointerDown(PointerEventData eventData)
        {
			//Debug.Log ("OnPointerDown 111111111111111 pos="+eventData.position);
			joyStickParent.transform.localPosition = eventData.position;
            // When we press, we first want to snap the joystick to the user's finger
            if (SnapsToFinger)
            {

//				Debug.Log ("OnPointerDown 222222222222");
                CurrentEventCamera = eventData.pressEventCamera ?? CurrentEventCamera;
            ...
            {
//				Debug.Log ("OnPointerDown 333333333");
				_initialStickPosition = _stickTransform.anchoredPosition;
				_intermediateStickPosition = _initialStickPosition;
				_initialBasePosition = _baseTransform.anchoredPosition;
				...
				// Then we change it's actual position so it snaps to the user's finger
//				Debug.Log("worldJoystickPosition="+worldJoystickPosition);
				...
				JoystickBase.transform.position = worldJoystickPosition;
                OnDrag(eventData);
            }
